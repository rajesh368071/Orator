...
public class TC023 extends AgentSuit {
	
	
	public void verifyChatBoxOpen() throws InterruptedException, AWTException, IOException {
		log.info("Verify Break In Button Displayed");
		//ExtentTestManager.startTest("(23) Verify Break In Button Displayed");
        ...
        boolean isBreakInDisplayed = ElementDisplayed(supervisor.L_breakin);
		
		if(isBreakInDisplayed) {
        	assertEquals(isBreakInDisplayed, true);
        	log.info(isBreakInDisplayed+"-"+true);
			ExtentSuccessMessage("Chat Escalated Successfully");
        }else {
        	log.info(isBreakInDisplayed+"-"+true);
        	ExtentErrorMessage("In Progress Chat Not Opened");
        }
